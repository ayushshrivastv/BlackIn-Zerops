...
export default function ContractTemplates() {
    ...
    function handleStartBuilding(template: Template | null = activeTemplate) {
        if (!session?.user.id) {
            return;
        }

        const contractId = uuid();
        set_states(contractId, null, template?.id, template ?? undefined, {
            markLoading: true,
        });
    }
    ...
    return (
        <div className="w-full h-full flex flex-col px-2 tracking-wider">
            <div className="w-full flex justify-between py-1 text-sm px-1">
                <span className="text-light">Featured app templates</span>
            </div>
                    ...
                    >
                        ...
                        <button
                            ...
                            onClick={() => {
                                ...
                                handleStartBuilding(template);
                            }}
                            className="absolute flex min-h-10 items-center justify-center gap-x-1 px-5 py-1 bg-primary text-light font-bold opacity-0 group-hover:opacity-100 transition-all ease-in-out rounded-[4px] bottom-12 right-3 translate-x-2 group-hover:translate-x-0 cursor-pointer focus-visible:ring-2 focus-visible:ring-primary-light focus-visible:ring-offset-2 focus-visible:ring-offset-darkest"
                        >
